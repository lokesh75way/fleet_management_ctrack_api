...
export const getFleetStatus = async (
  ...
  try {
    ...
    const completeCount = await Trip.countDocuments({
    ...
    const cancelledCount = await Trip.countDocuments({
...
export const getFleetUsage = async (
  ...
  const groupedByCountry = groups.reduce(
    (acc: { country: string; count: number; percentage: number }[], group) => {
...
export const getMaintainanceReminder = async (
  ...
  res.send(
    createResponse(
      maintainanceReminder,
      "Maintainance reminder fetched successfully!"
    )
  );
};
...
export const getRenewalReminder = async (
  ...
  res.send(
    createResponse(renewalReminder, "Renewal reminder fetched successfully!")
  );
};
